...
export const getPhones = (state, ownProps)=> {
  ...
  const applySearch  = item => R.includes( //проверка наличия первого аргумента во втором
...
export const getBasketPhonesWithCount = state => {
  ...
  const phones = R.compose(
    R.map(phoneWithCount), //добавляем каждому телефону поле count
    R.map(id => getPhoneById(state, id)) //получем телефоны по уникальным id
  )(uniqIds)
